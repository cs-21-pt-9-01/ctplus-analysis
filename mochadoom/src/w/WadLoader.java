...
public class WadLoader implements IWadLoader {
    ...
    public WadLoader() {
        ...
        zone = new org.apache.commons.collections4.map.HashedMap<CacheableDoomObject, Integer>();
        wadfiles = new org.apache.commons.collections4.list.TreeList<>();
    ...
    private void AddFile(String uri, ZipEntry entry, int type) throws Exception {
        ...
        int startlump;
        // MAES: was *
        filelump_t[] fileinfo = new filelump_t[1];
        ...
        for (int i = startlump; i < numlumps; i++, lump_p++, fileinfo_p++) {
            ...
            // MAES: Add Boom provenience info
            lumpinfo[lump_p].wadfile = wadinfo;
        }
        if (reloadname != null)
            handle.close();
    }
        ...
        for (String s : filenames) {
            if (s != null) {
                if (C2JUtils.testReadAccess(s)) {
                    ...
                    System.out.printf("\tadded %s (zipped: %s network: %s)\n", s, C2JUtils.flags(type, InputStreamSugar.ZIP_FILE), C2JUtils.flags(type, InputStreamSugar.NETWORK_FILE));
                } else
    ...
    public final void ReadLump(int lump, byte[] buf, int offset) {
        ...
        try {
            ...
            while (c < l.size) c += bis.read(buf, offset + c, (int) (l.size - c));
            if (c < l.size)
     ...
     */
    java.util.Map<String, Integer> doomhash;

    protected void InitLumpHash() {
        doomhash = new org.apache.commons.collections4.map.HashedMap<String, Integer>(numlumps);
    ...
    private final java.util.List<Integer> list = new org.apache.commons.collections4.list.TreeList<Integer>();
    ...
    public void CloseAllHandles() {
        java.util.List<InputStream> d = new org.apache.commons.collections4.list.TreeList<InputStream>();
    ...
    public int CoalesceMarkedResource(String start_marker, String end_marker, li_namespace namespace) {
        ...
        for (int i = 0; i < numlumps; i++) {
            lump = lumpinfo[i];
            if (// start marker found
            IsMarker(start_marker, lump.name)) {
                // start_marker,i);
                if (num_marked == 0) {
                    marked[num_marked] = new lumpinfo_t();
                    marked[num_marked].name = new String(start_marker);
                    // killough 3/20/98: force size to be 0
                    marked[num_marked].size = 0;
                    // killough 4/17/98
                ...
                // start marking lumps
                is_marked = true;
            } else if (// end marker found
            IsMarker(end_marker, lump.name)) {
                // System.err.printf("%s identified as end mark for %s index %d\n",lump.name,
                // end_marker,i);
                // add end marker below
                mark_end = true;
                // stop marking lumps
                is_marked = false;
                ...
                lumpinfo[num_unmarked++] = lump.clone();
        }
        ...
        mark_end) {
            ...
            // killough 4/17/98
            lumpinfo[numlumps].namespace = li_namespace.ns_global;
    ...
    public final static boolean IsMarker(String marker, String name) {
        ...
        if (name == null || name.length() == 0)
            return false;
        boolean result = // FF_* is valid alias for F_*, but HI_* should not allow HHI_*
        name.equalsIgnoreCase(marker) || (marker.charAt(1) == '_' && name.charAt(0) == marker.charAt(0) && name.substring(1).equalsIgnoreCase(marker));
        return result;
    }
    ...
    HashedMap<CacheableDoomObject, Integer> zone;
