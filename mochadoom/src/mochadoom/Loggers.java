...
public class Loggers {
    ...
    private static final java.util.Map<String, Logger> INDIVIDUAL_CLASS_LOGGERS = new org.apache.commons.collections4.map.HashedMap<>();
    ...
    public static <EventHandler extends Enum<EventHandler> & EventBase<EventHandler>> void LogEvent(final Logger logger, final ActionStateHolder<EventHandler> actionStateHolder, final EventHandler handler, final AWTEvent event) {
        if (!logger.isLoggable(Level.ALL) && lastHandler == handler) {
            return;
        }
        lastHandler = handler;
        final IntFunction<EventBase<EventHandler>[]> arrayGenerator = EventBase[]::new;
        final EventBase<EventHandler>[] depends = actionStateHolder.cooperations(handler, RelationType.DEPEND).stream().filter(hdl -> actionStateHolder.hasActionsEnabled(hdl, ActionMode.DEPEND)).toArray(arrayGenerator);
        final Map<RelationType, Set<EventHandler>> adjusts = actionStateHolder.adjustments(handler);
        final EventBase<EventHandler>[] causes = actionStateHolder.cooperations(handler, RelationType.CAUSE).stream().filter(hdl -> actionStateHolder.hasActionsEnabled(hdl, ActionMode.DEPEND)).toArray(arrayGenerator);
        final EventBase<EventHandler>[] reverts = actionStateHolder.cooperations(handler, RelationType.REVERT).stream().filter(hdl -> actionStateHolder.hasActionsEnabled(hdl, ActionMode.DEPEND)).toArray(arrayGenerator);
