...
public class SpriteManager<T, V> implements ISpriteManager {
    ...
    protected final void InitSpriteDefs(String[] namelist) {
        ...
        java.util.Map<Integer, List<Integer>> hash;
        int i;
        ...
        // the sprite name)
        // allocate hash table
        hash = new org.apache.commons.collections4.map.HashedMap<>(numentries);
            ...
            if (list != null && !list.isEmpty()) {
                ...
                list.forEach((j) -> {
                    ...
                    if (lump.name.substring(0, 4).equalsIgnoreCase(spritename.substring(0, 4))) {
                        ...
                        InstallSpriteLump(j + firstspritelump, frame, rotation, false);
                        if (lump.name.length() >= 7) {
                            frame = lump.name.charAt(6) - 'A';
                            rotation = lump.name.charAt(7) - '0';
                            InstallSpriteLump(j + firstspritelump, frame, rotation, true);
                        }
                    }
                });
                // check the frames that were found for completeness
                if (// 1/31/98
                (sprites[i].numframes = ++maxframe) != 0) {
                    int frame;
                    for (frame = 0; frame < maxframe; frame++) switch(sprtemp[frame].rotate) {
                        case -1:
                            // no rotations were found for that frame at all
                            DOOM.doomSystem.Error("R_InitSprites: No patches found for %s frame %c", namelist[i], frame + 'A');
                            break;
                        case 0:
                        ...
                        case 1:
                            // must have all 8 frames
                            {
                                ...
                                for (rotation = 0; rotation < 8; rotation++) if (sprtemp[frame].lump[rotation] == -1)
                                    DOOM.doomSystem.Error("R_InitSprites: Sprite %s frame %c is missing rotations", namelist[i], frame + 'A');
                                break;
                            }
                    }
    ...
    public void InitSpriteLumps() {
        ...
        for (i = 0; i < numspritelumps; i++) {
            ...
            patch = DOOM.wadLoader.CacheLumpNum(firstspritelump + i, PU_CACHE, patch_t.class);
    ...
    public final void InstallSpriteLump(int lump, int frame, int rotation, boolean flipped) {
        if (frame >= MAX_SPRITE_FRAMES || rotation > 8)
            DOOM.doomSystem.Error("R_InstallSpriteLump: Bad frame characters in lump %d", lump);
        if (frame > maxframe) {
            maxframe = frame;
        }
        if (rotation == 0) {
            ...
            int r;
            for (r = 0; r < 8; r++) if (sprtemp[frame].lump[r] == -1) {
                sprtemp[frame].lump[r] = lump - firstspritelump;
                sprtemp[frame].flip[r] = (byte) (flipped ? 1 : 0);
                // jff 4/24/98 if any subbed,
        ...
        if (sprtemp[frame].lump[--rotation] == -1) {
            ...
            // jff 4/24/98 only change if rot
