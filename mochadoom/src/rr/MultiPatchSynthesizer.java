...
public class MultiPatchSynthesizer {
    ...
    public static patch_t synthesizePatchFromFlat(String name, byte[] flat, int width, int height) {
        ...
        for (int x = 0; x < width; x++) columns[x] = getColumnStream(pixels[x], solid[x], height);
    ...
    public static column_t getColumnStream(byte[] pixels, boolean[] solid, int height) {
        ...
        int start = -1;
        int end = -1;
        List<PixelRange> ranges = new org.apache.commons.collections4.list.TreeList<PixelRange>();
        ...
        for (int i = 0; i < height; i++) {
            ...
            if (!solid[i] && start != -1 && end == -1) {
                // Single-pixel runs would be e.g. 1-2 -> 1-1
                end = i - 1;
            }
            if (start != -1 && end != -1) {
                // Range complete.
        ...
        for (int i = 0; i < n; i++) {
            ...
            // cumulative top delta
            topdelta = pr.start;
            // Precomputed column data
            // Last written post +3, pointing at first pixel of data.
            postofs[i] = (short) file.size() + 3;
            topdeltas[i] = (short) topdelta;
            // Post lengths are at net of padding
