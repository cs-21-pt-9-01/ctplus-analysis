...
public class DavidSFXModule extends AbstractSoundDriver {
    ...
    java.util.List<DoomSound> cachedSounds = new org.apache.commons.collections4.list.TreeList<DoomSound>();
    ...
    private final void createDataLineForChannel(int c, int sfxid) {
        if (channels[c].auline == null) {
            ...
            if (channels[c].auline.isControlSupported(Type.MASTER_GAIN))
                channels[c].vc = (FloatControl) channels[c].auline.getControl(Type.MASTER_GAIN);
            else {
                System.err.print("MASTER_GAIN, ");
                errors = true;
                if (channels[c].auline.isControlSupported(Type.VOLUME))
                    channels[c].vc = (FloatControl) channels[c].auline.getControl(Type.VOLUME);
                else
	...
	*/
    ...
    protected int addsfx(int sfxid, int volume, int pitch, int seperation) {
        ...
        if (// was !handlenums, so it's actually 1...100?
        handlenums == 0)
    ...
    private class SoundWorker implements Runnable {
        ...
        // Holds the worker still until there's a new sound
        public Semaphore wait;

        // linear volume control
        FloatControl vc;

        // balance/panning control
        FloatControl bc;
